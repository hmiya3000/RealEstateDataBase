...
@interface InputLandAreaViewCtrl ()
{
    CGFloat             _value;
    UILabel             *_l_landArea;
    UITextView          *_tv_tips;
...
@implementation InputLandAreaViewCtrl
...
- (void)viewDidLoad
{
    ...
    _value  = _modelRE.estate.land.area;
...
- (void)viewMake
{
    ...
    if ( _pos.isPortrait == true ){
        [UIUtil setRectLabel:_l_landArea   x:pos_x     y:pos_y viewWidth:_pos.len30 viewHeight:dy  color:[UIUtil color_Ivory] ];
        pos_y = pos_y + dy;
        _tv_tips.frame = CGRectMake(pos_x, pos_y, _pos.len30, dy*1.2);
    }else {
        [UIUtil setRectLabel:_l_landArea    x:pos_x     y:pos_y viewWidth:_pos.len15 viewHeight:dy  color:[UIUtil color_Ivory] ];
        pos_y = pos_y + dy;
...
-(void)clickButton:(UIButton*)sender
{
    ...
    _modelRE.estate.land.area  = _value;
    [_modelRE valToFile];
...
{
    _value              = value;
    _l_landArea.text       = [NSString stringWithFormat:@"%g ㎡",_value];
