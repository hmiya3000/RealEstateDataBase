...
@interface InputLoanBorrowViewCtrl ()
{
...
@implementation InputLoanBorrowViewCtrl
...
-(void)viewDidLoad
{
    ...
    _value  = _modelRE.investment.loan.loanBorrow /10000;
    ...
    /****************************************/
    _l_loanBorrow   = [UIUtil makeLabel:[NSString stringWithFormat:@"%@万円",[UIUtil yenValue:_value]]];
    ...
    [_l_selfFinance setTextAlignment:NSTextAlignmentLeft];
    [_scrollView addSubview:_l_selfFinance];
    /****************************************/
    _l_selfFinanceVal   = [UIUtil makeLabel:@"999万円"];
    [_l_selfFinanceVal setTextAlignment:NSTextAlignmentRight];
    [_scrollView addSubview:_l_selfFinanceVal];
    /****************************************/
    ...
    _tv_tips.text           = [NSString stringWithFormat:@"借入金を入力すると必要な自己資金を算出します\n借地権は融資がつきにくい傾向にあります"];
...
-(void)viewMake{
    ...
    if ( _pos.isPortrait == true ){
        [UIUtil setRectLabel:_l_loanBorrow    x:pos_x     y:pos_y viewWidth:_pos.len30 viewHeight:dy  color:[UIUtil color_Ivory] ];
        pos_y = pos_y + dy;
        ...
        pos_y = pos_y + dy*0.65;
        [UIUtil setLabel:_l_selfFinance     x:pos_x         y:pos_y length:_pos.len10];
        [UIUtil setLabel:_l_selfFinanceVal  x:pos_x+dx*2    y:pos_y length:_pos.len10];
        pos_y = pos_y + dy*0.6;
        _tv_tips.frame = CGRectMake(pos_x, pos_y, _pos.len30, dy*1.7);
    }else {
        [UIUtil setRectLabel:_l_loanBorrow    x:pos_x     y:pos_y viewWidth:_pos.len15 viewHeight:dy  color:[UIUtil color_Ivory] ];
        pos_y = pos_y + dy;
        ...
        pos_y = pos_y + dy;
        [UIUtil setLabel:_l_selfFinance     x:pos_x         y:pos_y length:_pos.len15/2];
        [UIUtil setLabel:_l_selfFinanceVal  x:pos_x+dx*0.75 y:pos_y length:_pos.len15/2];
        pos_y = pos_y + dy;
...
{
    _value              = value;
    _l_loanBorrow.text  = [NSString stringWithFormat:@"%@万円",[UIUtil yenValue:_value]];
    NSInteger priceAll  = _modelRE.investment.price+_modelRE.investment.expense;
    _l_priceVal.text    = [NSString stringWithFormat:@"%@万円",[UIUtil yenValue:priceAll/10000]];
    _l_selfFinanceVal.text  = [NSString stringWithFormat:@"%@万円",[UIUtil yenValue:(priceAll/10000-_value)]];
