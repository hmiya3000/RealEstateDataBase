...
#import "InvestAnalysisViewCtrl.h"
#import "UIUtil.h"
#import "ModelRE.h"
#import "Pos.h"

@interface InvestAnalysisViewCtrl()
{
    ModelRE             *_modelRE;
...
@implementation InvestAnalysisViewCtrl
//======================================================================
//
//======================================================================
- (id)init
...
-(void)viewMake{
    ...
    if ( _pos.isPortrait == true ){
        ...
        /*--------------------------------------*/
        pos_y = pos_y + dy;
        _tv_comment2.frame      = CGRectMake(pos_x,         pos_y, length30, dy*5);
        /*--------------------------------------*/
    }else {
        [UIUtil setRectLabel:_l_name x:pos_x y:pos_y viewWidth:length30 viewHeight:dy color:[UIUtil color_WakatakeIro]];
        pos_y = pos_y + dy;
...
-(void)rewriteProperty
{
    ...
    _tv_comment1.text           = [NSString stringWithFormat:@"キャップレート　 = 営業利益 / 物件価格\n総収益率　　　　 = 営業利益 / (自己資金 + 借入金)\nローン定数　　　 = 年間返済額 / 借入金\n自己資本配当率　 = 税引前CF / 自己資金\n自己資本回収年数 = 自己資金 / 税引前CF\n\n"];
    if( _modelRE.ope1.fcr > _modelRE.ope1.loanConst ){
        _tv_comment1.text = [_tv_comment1.text stringByAppendingFormat:@"・総収益率:%2.2f%% > ローン定数%2.2f%%\n　　→投資として適格です\n",_modelRE.ope1.fcr*100,_modelRE.ope1.loanConst*100];
        if( _modelRE.ope1.ccr > _modelRE.ope1.fcr ){
            _tv_comment1.text = [_tv_comment1.text stringByAppendingFormat:@"・自己資本配当率:%2.2f%% > 総収益率:%2.2f%% > ローン定数%2.2f%%\n　　→借入によるレバレッジが効いています\n",_modelRE.ope1.ccr*100,_modelRE.ope1.fcr*100,_modelRE.ope1.loanConst*100];
            _tv_comment1.text = [_tv_comment1.text stringByAppendingFormat:@"・投入した自己資金は%2.1f年で回収できます\n",_modelRE.ope1.pb];
        } else {
            _tv_comment1.text = [_tv_comment1.text stringByAppendingFormat:@"・総収益率:%2.2f%% > 自己資本配当率:%2.2f%%\n　　→借入によるレバレッジが効いていません\n",_modelRE.ope1.fcr*100,_modelRE.ope1.ccr*100];
        }
    } else {
        _tv_comment1.text = [_tv_comment1.text stringByAppendingFormat:@"・ローン定数%2.2f%% > 総収益率:%2.2f%%\n　　→損しています。投資として不適格です\n",_modelRE.ope1.loanConst*100,_modelRE.ope1.fcr*100];

    }
    _tv_comment2.text           = [NSString stringWithFormat:@"借入償還余裕率 = 営業利益 / 年間返済額\n損益分岐入居率 = (運営費 + 年間返済額) / 潜在総収入\n借入金割合　　 = 借入金 / 物件価格\n\n"];
    if( _modelRE.ope1.dcr >= 1.3 ){
        _tv_comment2.text = [_tv_comment2.text stringByAppendingFormat:@"・借入償還余裕率:%2.2f > 1.3\n　　→一般的な目安以上の余裕があります\n",_modelRE.ope1.dcr];
    } else if ( _modelRE.ope1.dcr < 1.3 &&  _modelRE.ope1.dcr >= 1.2 ){
        _tv_comment2.text = [_tv_comment2.text stringByAppendingFormat:@"・借入償還余裕率:%2.2f < 1.3\n　　→余裕率は一般的な目安以下です\n",_modelRE.ope1.dcr];
    } else {
        _tv_comment2.text = [_tv_comment2.text stringByAppendingFormat:@"・借入償還余裕率:%2.2f < 1.2\n　　→この水準では銀行は融資しないと一般では言われています\n",_modelRE.ope1.dcr];
    }
    if( _modelRE.ope1.ber < 0.7 ){
        _tv_comment2.text = [_tv_comment2.text stringByAppendingFormat:@"・損益分岐入居率:%2.2f%% < 70%%\n　　→空室リスクに対して十分な余裕があります\n",_modelRE.ope1.ber*100];
    } else {
        _tv_comment2.text = [_tv_comment2.text stringByAppendingFormat:@"・損益分岐入居率:%2.2f%% > 70%%\n　　→空室リスクに耐えられない可能性があります\n",_modelRE.ope1.ber*100];
    }
    if( _modelRE.ope1.ltv > 0.8 ){
        _tv_comment2.text = [_tv_comment2.text stringByAppendingFormat:@"・借入金割合:%2.2f%% ≧ 80%%\n　　→借入の割合が高めです\n",_modelRE.ope1.ltv*100];
    } else if( _modelRE.ope1.ltv <= 0.8 && _modelRE.ope1.ltv >= 0.6 ){
        _tv_comment2.text = [_tv_comment2.text stringByAppendingFormat:@"・60%% ≦ 借入金割合:%2.2f%% ≦ 80%%\n　　→借入の割合は適正です\n",_modelRE.ope1.ltv*100];
    } else {
        _tv_comment2.text = [_tv_comment2.text stringByAppendingFormat:@"・借入金割合:%2.2f%% < 60%%\n　　→借入の割合は低めです\n",_modelRE.ope1.ltv*100];
    }
