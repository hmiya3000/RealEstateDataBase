...
- (NSInteger) getAmortizationCosts_term:(NSInteger)term
{
    ...
    NSInteger usefulLifeTermHouse   = [House usefulLifeYear:_construct]*12;     //耐用年数(躯体)
    NSInteger usefulLifeTermEquip   = 15 *12;                                   //耐用年数(設備)
    NSInteger amPeriodYearHouse     = [House getAmortizationPeriod_elapsedTerm:elapsedTerm usefulLifeTerm:usefulLifeTermHouse];
    NSInteger amPeriodYearEquip     = [House getAmortizationPeriod_elapsedTerm:elapsedTerm usefulLifeTerm:usefulLifeTermEquip];
    NSInteger priceBookEquip        = _priceBook * _equipRatio;
    NSInteger priceBookHouse        = _priceBook - priceBookEquip;
    

    //減価償却費の計算
    NSInteger amCosts;
    //躯体
    if ( 0 <= term && term < amPeriodYearHouse*12 ){
        amCosts     = priceBookHouse * amRate[amPeriodYearHouse];
    } else {
        // 償却期間外
        amCosts     = 0;
    }
    //設備
    if ( 0 <= term && term < amPeriodYearEquip*12 ){
        amCosts     += priceBookEquip * amRate[amPeriodYearEquip];
    } else {
        // 償却期間外
        amCosts     += 0;
    }
...
- (NSInteger)getAmortizationCostsSum_period:(NSInteger)period
{
    ...
    for(int i=0; i< period; i++){
        amCosts = amCosts + [self getAmortizationCosts_term:i];
    }
    return amCosts;
}
//======================================================================
...
-(void)calcValuation
{
    _valuation = [self getValuation_term:0];
}
//======================================================================
// 不動産取得税の計算
//======================================================================
